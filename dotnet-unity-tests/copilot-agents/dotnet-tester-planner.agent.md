...
description: Particiona o `test-plan.md` gerado pelo dotnet-tester-reviewer em arquivos `<NomeProjeto>.plan.md` individuais por unidade paralelizável, com metadados de dependência derivados da seção 4 do plano mestre. Não escreve código de teste nem roda builds. Ativa para "particionar plano", "quebrar plano de testes", "gerar plans individuais", "breakdown do test-plan".
...
1. Leia `<session-dir>/test-plan.md`.
...
Para cada projeto, crie `<session-dir>/<NomeProjeto>.plan.md` com a seguinte estrutura:
...
1. Ao terminar, liste `<session-dir>/*.plan.md` para confirmar a geração.
